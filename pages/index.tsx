  ...
  return {
    props: {
      hero: await client
        ...
        .then((res: EntryCollection<ISectionHero>) => res.items[0]),
      about: await client
        .getEntries({ content_type: "sectionAbout", locale })
        .then((res: EntryCollection<ISectionAbout>) => res.items[0]),
      layout: await client
        .getEntries({ content_type: "layout", locale })
        .then((res: EntryCollection<ILayout>) => res.items[0]),
      ourValues: await client
        .getEntries({ content_type: "sectionOurValues", locale })
        .then((res: EntryCollection<ISectionOurValues>) => res.items[0]),
    },
  };
}
